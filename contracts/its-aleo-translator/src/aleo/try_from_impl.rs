...
impl<N: ::snarkvm_cosmwasm::prelude::Network> TryFrom<&ItsOutgoingInterchainTransfer<N>>
    for ::snarkvm_cosmwasm::prelude::Plaintext<N>
{
    type Error = ::anyhow::Error;
    fn try_from(value: &ItsOutgoingInterchainTransfer<N>) -> ::anyhow::Result<Self, Self::Error> {
...
impl<N: ::snarkvm_cosmwasm::prelude::Network> TryFrom<&RemoteDeployInterchainToken>
    for ::snarkvm_cosmwasm::prelude::Plaintext<N>
{
    type Error = ::anyhow::Error;
    fn try_from(value: &RemoteDeployInterchainToken) -> ::anyhow::Result<Self, Self::Error> {
        let mut map = ::indexmap::IndexMap::new();
        {
            map.insert(
                "payload".parse()?,
                ::snarkvm_cosmwasm::prelude::Plaintext::try_from(&value.payload)?,
            );
        }
        {
            map.insert("destination_chain".parse()?, {
    ...
    fn try_from(
        ...
        let inner_message = {
            ...
            let result: ::anyhow::Result<IncomingInterchainTransfer<N>> = {
                let member_name = "inner_message";
                Ok(IncomingInterchainTransfer::try_from(value).map_err(|e| {
                    ::anyhow::anyhow!(
                        "Failed to convert Plaintext to {}({}): {}",
                        stringify!(IncomingInterchainTransfer),
                        member_name,
                        e
                    )
                })?)
        ...
        let source_chain = {
            let key: ::snarkvm_cosmwasm::prelude::Identifier<N> = "source_chain".parse()?;
            let value = map
                .get(&key)
...
impl<N: ::snarkvm_cosmwasm::prelude::Network> TryFrom<&OutgoingInterchainTransfer<N>>
    for ::snarkvm_cosmwasm::prelude::Plaintext<N>
{
    type Error = ::anyhow::Error;
    fn try_from(value: &OutgoingInterchainTransfer<N>) -> ::anyhow::Result<Self, Self::Error> {
        let mut map = ::indexmap::IndexMap::new();
        {
            map.insert("its_token_id".parse()?, {
                let elements = value
        ...
        }
        {
            map.insert(
                ...
                ::snarkvm_cosmwasm::prelude::Plaintext::from(
                    ::snarkvm_cosmwasm::prelude::Literal::Address(value.source_address),
                ),
            );
        }
        {
    ...
    fn try_from(
        ...
        let inner_message = {
            ...
            let result: ::anyhow::Result<FromRemoteDeployInterchainToken<N>> = {
                let member_name = "inner_message";
                Ok(
                    FromRemoteDeployInterchainToken::try_from(value).map_err(|e| {
                        ::anyhow::anyhow!(
                            "Failed to convert Plaintext to {}({}): {}",
                            stringify!(FromRemoteDeployInterchainToken),
                            member_name,
                            e
                        )
                    })?,
...
impl<N: ::snarkvm_cosmwasm::prelude::Network> TryFrom<&DeployInterchainToken>
    for ::snarkvm_cosmwasm::prelude::Plaintext<N>
{
    type Error = ::anyhow::Error;
