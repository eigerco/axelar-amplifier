...
impl Base58SolanaTxDigestAndEventIndex {
    pub fn tx_digest_as_base58(&self) -> nonempty::String {
        nonempty::String::from_str(&self.signature).unwrap()
    }

    pub fn new(signature: String, event_index: impl Into<u32>) -> Self {
        Self {
            signature,
...
impl Display for Base58SolanaTxDigestAndEventIndex {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}-{}", self.signature, self.event_index)
    }
}
...
mod tests {
    ...
    fn should_parse_msg_id() {
        let res = Base58SolanaTxDigestAndEventIndex::from_str(
            "3Gwj2GFJGeaVPMnRFLChBmmtYrxP5xqhCTD3DMmpVyfD-0",
        );
        assert!(res.is_ok());
